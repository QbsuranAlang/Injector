//
//  IJTNULLScanResultTableViewController.m
//  Injector
//
...
@interface IJTNULLScanResultTableViewController ()
...
@property (nonatomic, strong) IJTNULL_Scan *null_scan;

@end

@implementation IJTNULLScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"NULL Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"NULL ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.null_scan = [[IJTNULL_Scan alloc] init];
    [self.null_scan setTarget:self.target];
    if(self.null_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.null_scan.errorCode)]];
        return;
    }
    [self.null_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [_null_scan injectWithInterval:_interval
                     randomization:_randomization
                              stop:&_cancle
                           timeout:_timeout
                            target:self
                          selector:FINSCAN_CALLBACK_SEL
                            object:_replyArray];
    
    if(_null_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_null_scan.errorCode)]];
        }
    }
    ...
    [self.null_scan close];
    self.null_scan = nil;
    self.requestThread = nil;
}

FINSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        NSString *state = @"";
        if(flags == IJTNULL_ScanFlagsClose)
            state = @"Closed";
        else if(flags == (IJTNULL_ScanFlagsFiltered | IJTNULL_ScanFlagsOpen))
            state = @"Open or Filtered";
        else if(flags == IJTNULL_ScanFlagsFiltered)
            state = @"Filtered";
...
- (void)updateProgressView: (id)sender {
    float value = ([_null_scan getTotalInjectCount] - [_null_scan getRemainInjectCount])/(float)[_null_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_null_scan getTotalInjectCount] - ([_null_scan getTotalInjectCount] - [_null_scan getRemainInjectCount]);
