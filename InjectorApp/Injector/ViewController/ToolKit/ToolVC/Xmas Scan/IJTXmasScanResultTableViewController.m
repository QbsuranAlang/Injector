//
//  IJTXmasScanResultTableViewController.m
//  Injector
//
...
@interface IJTXmasScanResultTableViewController ()
...
@property (nonatomic, strong) IJTXmas_Scan *xmas_scan;

@end

@implementation IJTXmasScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Xmas Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"Xmas ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.xmas_scan = [[IJTXmas_Scan alloc] init];
    [self.xmas_scan setTarget:self.target];
    if(self.xmas_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.xmas_scan.errorCode)]];
        return;
    }
    [self.xmas_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [_xmas_scan injectWithInterval:_interval
                     randomization:_randomization
                              stop:&_cancle
                           timeout:_timeout
                            target:self
                          selector:XMASSCAN_CALLBACK_SEL
                            object:_replyArray];
    
    if(_xmas_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_xmas_scan.errorCode)]];
        }
    }
    ...
    [self.xmas_scan close];
    self.xmas_scan = nil;
    self.requestThread = nil;
}

XMASSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        NSString *state = @"";
        if(flags == IJTXmas_ScanFlagsClose)
            state = @"Closed";
        else if(flags == (IJTXmas_ScanFlagsFiltered | IJTXmas_ScanFlagsOpen))
            state = @"Open or Filtered";
        else if(flags == IJTXmas_ScanFlagsFiltered)
            state = @"Filtered";
...
- (void)updateProgressView: (id)sender {
    float value = ([_xmas_scan getTotalInjectCount] - [_xmas_scan getRemainInjectCount])/(float)[_xmas_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_xmas_scan getTotalInjectCount] - ([_xmas_scan getTotalInjectCount] - [_xmas_scan getRemainInjectCount]);
