//
//  IJTConnectScanResultTableViewController.m
//  Injector
//
...
#import "IJTConnectScanResultTableViewController.h"
#import "IJTScanTaskTableViewCell.h"
#import "IJTScanPortTableViewCell.h"

@interface IJTConnectScanResultTableViewController ()
...
@property (nonatomic, strong) IJTConnect_Scan *connect_scan;

@end

@implementation IJTConnectScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Connect Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"Connect ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.connect_scan = [[IJTConnect_Scan alloc] init];
    [self.connect_scan setTarget:self.target];
    if(self.connect_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.connect_scan.errorCode)]];
        return;
    }
    [self.connect_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [self.connect_scan connectScanStop:&_cancle
                         randomization:_randomization
                               timeout:_timeout
                              interval:_interval
                                target:self
                              selector:CONNECTSCAN_CALLBACK_SEL
                                object:_replyArray];
    
    if(_connect_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_connect_scan.errorCode)]];
        }
    }
    ...
    [IJTDispatch dispatch_main:^{
        ...
        for(int i = 0 ; i < self.replyArray.count ; i++) {
            for(int j = 0 ; j < i ;j++) {
                NSDictionary *dict1 = [self.replyArray objectAtIndex:i];
                NSDictionary *dict2 = [self.replyArray objectAtIndex:j];
                NSNumber *portNumber1 = [dict1 valueForKey:@"Port"];
                NSNumber *portNumber2 = [dict2 valueForKey:@"Port"];
                if([portNumber1 unsignedShortValue] < [portNumber2 unsignedShortValue]) {
                    [self.replyArray exchangeObjectAtIndex:i withObjectAtIndex:j];
                }
            }
        }//end for sort
        [self.tableView beginUpdates];
...
CONNECTSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        if(flags == IJTConnect_ScanFlagsClose)
            [dict setValue:@"Closed" forKey:@"State"];
        else if(flags == IJTConnect_ScanFlagsOpen)
            [dict setValue:@"Open" forKey:@"State"];
...
- (void)updateProgressView: (id)sender {
    float value = ([_connect_scan getTotalInjectCount] - [_connect_scan getRemainInjectCount])/(float)[_connect_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_connect_scan getTotalInjectCount] - ([_connect_scan getTotalInjectCount] - [_connect_scan getRemainInjectCount]);
...
- (NSArray *)allStringsForProgressView:(ASProgressPopUpView *)progressView {
    ...
    return @[@"Try connecting...", s, @"Completed"];
}
