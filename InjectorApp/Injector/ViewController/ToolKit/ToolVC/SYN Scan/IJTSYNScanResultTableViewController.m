//
//  IJTSYNScanResultTableViewController.m
//  Injector
//
...
@interface IJTSYNScanResultTableViewController ()
...
@property (nonatomic, strong) IJTSYN_Scan *syn_scan;

@end

@implementation IJTSYNScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"SYN Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"SYN ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.syn_scan = [[IJTSYN_Scan alloc] init];
    [self.syn_scan setTarget:self.target];
    if(self.syn_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.syn_scan.errorCode)]];
        return;
    }
    [self.syn_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [_syn_scan injectWithInterval:_interval
                    randomization:_randomization
                             stop:&_cancle
                          timeout:_timeout
                           target:self
                         selector:SYNSCAN_CALLBACK_SEL
                           object:_replyArray];
    
    if(_syn_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_syn_scan.errorCode)]];
        }
    }
...
SYNSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        NSString *state = @"";
        if(flags == IJTSYN_ScanFlagsClose)
            state = @"Closed";
        else if(flags == IJTSYN_ScanFlagsFiltered)
            state = @"Filtered";
        else if(flags == IJTSYN_ScanFlagsOpen)
            state = @"Open";
...
- (void)updateProgressView: (id)sender {
    float value = ([_syn_scan getTotalInjectCount] - [_syn_scan getRemainInjectCount])/(float)[_syn_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_syn_scan getTotalInjectCount] - ([_syn_scan getTotalInjectCount] - [_syn_scan getRemainInjectCount]);
