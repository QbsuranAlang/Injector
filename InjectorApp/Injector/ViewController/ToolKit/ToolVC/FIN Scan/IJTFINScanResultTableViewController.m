//
//  IJTFINScanResultTableViewController.m
//  Injector
//
...
@interface IJTFINScanResultTableViewController ()
...
@property (nonatomic, strong) IJTFIN_Scan *fin_scan;

@end

@implementation IJTFINScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"FIN Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"FIN ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.fin_scan = [[IJTFIN_Scan alloc] init];
    [self.fin_scan setTarget:self.target];
    if(self.fin_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.fin_scan.errorCode)]];
        return;
    }
    [self.fin_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [_fin_scan injectWithInterval:_interval
    ...
    if(_fin_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_fin_scan.errorCode)]];
        }
    }
...
FINSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        NSString *state = @"";
        if(flags == IJTFIN_ScanFlagsClose)
            state = @"Closed";
        else if(flags == (IJTFIN_ScanFlagsFiltered | IJTFIN_ScanFlagsOpen))
            state = @"Open or Filtered";
        else if(flags == IJTFIN_ScanFlagsFiltered)
            state = @"Filtered";
...
- (void)updateProgressView: (id)sender {
    float value = ([_fin_scan getTotalInjectCount] - [_fin_scan getRemainInjectCount])/(float)[_fin_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_fin_scan getTotalInjectCount] - ([_fin_scan getTotalInjectCount] - [_fin_scan getRemainInjectCount]);
