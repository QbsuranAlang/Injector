//
//  IJTUDPScanResultTableViewController.m
//  Injector
//
...
#import "IJTUDPScanResultTableViewController.h"
#import "IJTScanTaskTableViewCell.h"
#import "IJTScanPortTableViewCell.h"

@interface IJTUDPScanResultTableViewController ()
...
@property (nonatomic, strong) IJTUDP_Scan *udp_scan;

@end

@implementation IJTUDPScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"UDP Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"UDP ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.udp_scan = [[IJTUDP_Scan alloc] init];
    [self.udp_scan setTarget:self.target];
    if(self.udp_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.udp_scan.errorCode)]];
        return;
    }
    [self.udp_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [_udp_scan injectWithInterval:_interval
                    randomization:_randomization
                             stop:&_cancle
                          timeout:_timeout
                           target:self
                         selector:UDPSCAN_CALLBACK_SEL
                           object:_replyArray];
    
    if(_udp_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_udp_scan.errorCode)]];
        }
    }
...
UDPSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        if(flags == IJTUDP_ScanFlagsClose)
            state = @"Closed";
        else if(flags == (IJTUDP_ScanFlagsOpen | IJTUDP_ScanFlagsFiltered))
            state = @"Open or Filtered";
        else if(flags == IJTUDP_ScanFlagsOpen)
            state = @"Open";
        else if(flags == IJTUDP_ScanFlagsFiltered)
            state = @"Filtered";
...
- (void)updateProgressView: (id)sender {
    float value = ([_udp_scan getTotalInjectCount] - [_udp_scan getRemainInjectCount])/(float)[_udp_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_udp_scan getTotalInjectCount] - ([_udp_scan getTotalInjectCount] - [_udp_scan getRemainInjectCount]);
...
- (NSArray *)allStringsForProgressView:(ASProgressPopUpView *)progressView {
    ...
    return @[@"Try connecting...", s, @"Double Checking..."];
}
