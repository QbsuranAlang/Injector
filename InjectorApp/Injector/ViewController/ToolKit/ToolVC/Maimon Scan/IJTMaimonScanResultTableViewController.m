//
//  IJTMaimonScanResultTableViewController.m
//  Injector
//
...
@interface IJTMaimonScanResultTableViewController ()
...
@property (nonatomic, strong) IJTMaimon_Scan *maimon_scan;

@end

@implementation IJTMaimonScanResultTableViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Maimon Scan";
    ...
    self.scanButton = [[UIBarButtonItem alloc]
                       initWithImage:[UIImage imageNamed:@"Maimon ScanNav.png"]
                       style:UIBarButtonItemStylePlain
...
- (void)scan {
    ...
    self.maimon_scan = [[IJTMaimon_Scan alloc] init];
    [self.maimon_scan setTarget:self.target];
    if(self.maimon_scan.errorHappened) {
        [self showErrorMessage:[NSString stringWithFormat:@"%s.", hstrerror(self.maimon_scan.errorCode)]];
        return;
    }
    [self.maimon_scan setStartPort:self.startPort endPort:self.endPort];
...
- (void)scanThread {
    int ret =
    [_maimon_scan injectWithInterval:_interval
                     randomization:_randomization
                              stop:&_cancle
                           timeout:_timeout
                            target:self
                          selector:MAIMONSCAN_CALLBACK_SEL
                            object:_replyArray];
    
    if(_maimon_scan.errorHappened) {
        if(ret == -2) {
            [self showErrorMessage:@"Host seems down."];
        }
        else {
            [self showErrorMessage:[NSString stringWithFormat:@"%s.", strerror(_maimon_scan.errorCode)]];
        }
    }
    ...
    [self.maimon_scan close];
    self.maimon_scan = nil;
    self.requestThread = nil;
}

MAIMONSCAN_CALLBACK_METHOD {
    [IJTDispatch dispatch_main:^{
        ...
        NSString *state = @"";
        if(flags == IJTMaimon_ScanFlagsClose)
            state = @"Closed";
        else if(flags == (IJTMaimon_ScanFlagsFiltered | IJTMaimon_ScanFlagsOpen))
            state = @"Open or Filtered";
        else if(flags == IJTMaimon_ScanFlagsFiltered)
            state = @"Filtered";
...
- (void)updateProgressView: (id)sender {
    float value = ([_maimon_scan getTotalInjectCount] - [_maimon_scan getRemainInjectCount])/(float)[_maimon_scan getTotalInjectCount];
...
- (NSString *)progressView:(ASProgressPopUpView *)progressView stringForProgress:(float)progress
{
    ...
    if(progress == 0) {
        ...
        u_int64_t count = [_maimon_scan getTotalInjectCount] - ([_maimon_scan getTotalInjectCount] - [_maimon_scan getRemainInjectCount]);
...
- (NSArray *)allStringsForProgressView:(ASProgressPopUpView *)progressView {
    ...
    return @[@"Try connecting...", s, @"Reading..."];
}
