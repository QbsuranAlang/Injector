...
@interface IJTFIN_Scan : NSObject

typedef NS_ENUM(u_int8_t, IJTFIN_ScanFlags) {
    IJTFIN_ScanFlagsClose = 0x00,
    IJTFIN_ScanFlagsOpen = 0x01,
    IJTFIN_ScanFlagsFiltered = 0x02
};

/**
 * fin scan callback function define
 * id self
 * SEL method
 * u_int16_t port
 * NSString *name
 ...
 * id object
 */
typedef void (*FINScanCallback)(id, SEL, u_int16_t, NSString *, IJTFIN_ScanFlags, id);

#define FINSCAN_CALLBACK_SEL @selector(finScanPort:portName:flags:object:)

#define FINSCAN_CALLBACK_METHOD \
- (void)finScanPort: (u_int16_t)port \
