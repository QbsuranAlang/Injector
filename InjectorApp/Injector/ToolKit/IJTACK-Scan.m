...
- (int)injectTarget: (NSString *)target
    ...
    in_addr_t dest = 0;
    inet_pton(AF_INET, [target UTF8String], &dest);
    ...
    make_tcp_header(&send_packet,
                    ...
                    1, 1, TH_ACK, datalen);
    make_ip_header(&(send_packet.ip), src_ip, sin.sin_addr.s_addr, iplen);
    
    if ((size = sendto(_sockfd, (char *)&send_packet, iplen, 0, (struct sockaddr *) &sin, sizeof(sin))) < 0) {
        goto BAD;
    }
    if(size != iplen)
        goto BAD;
    ...
    while(1) {
        ...
        while(bpf_len > 0) {
            ...
            if(tcp->th_sport == htons(port)) {
                ...
                [result addObject:[NSString stringWithUTF8String:ntop_buf]];
            }
