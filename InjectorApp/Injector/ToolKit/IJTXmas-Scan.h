...
@interface IJTXmas_Scan : NSObject

typedef NS_ENUM(u_int8_t, IJTXmas_ScanFlags) {
    IJTXmas_ScanFlagsClose = 0x00,
    IJTXmas_ScanFlagsOpen = 0x01,
    IJTXmas_ScanFlagsFiltered = 0x02
};

/**
 * xmas scan callback function define
 * id self
 * SEL method
 * u_int16_t port
 * NSString *name
 ...
 * id object
 */
typedef void (*XmasScanCallback)(id, SEL, u_int16_t, NSString *, IJTXmas_ScanFlags, id);

#define XMASSCAN_CALLBACK_SEL @selector(xmasScanPort:portName:flags:object:)

#define XMASSCAN_CALLBACK_METHOD \
- (void)xmasScanPort: (u_int16_t)port \
